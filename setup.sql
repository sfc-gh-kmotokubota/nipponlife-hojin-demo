...
snowflake.execute({sqlText:"INSERT INTO T_COMPANY_NEWS(NEWS_ID,COMPANY_ID,HEADLINE,BODY_TEXT,NEWS_DATE,EVENT_TYPE,INSURANCE_RELEVANCE,NEWS_SOURCE)VALUES('"+nidS+"','"+c.id+"','"+hl+"','"+bd+"','"+ds+"','"+et+"','"+rel+"','日経新聞')"});
if(ni<2&&(rel==='最高'||rel==='高')){var aidS='AL'+String(aid).padStart(4,'0');aid++;snowflake.execute({sqlText:"INSERT INTO T_EVENT_ALERTS(ALERT_ID,COMPANY_ID,EVENT_TYPE,EVENT_SUMMARY,INSURANCE_RELEVANCE,PRIORITY_LEVEL,RECOMMENDED_ACTION,RESPONSE_DEADLINE,STATUS,DETECTED_AT)VALUES('"+aidS+"','"+c.id+"','"+et+"','"+bd+"','"+rel+"','最高優先度','"+act+"',DATEADD(DAY,3,CURRENT_DATE()),'UNREAD',CURRENT_TIMESTAMP())"});}}}
...
snowflake.execute({sqlText:"INSERT INTO T_MEETINGS(MEETING_ID,COMPANY_ID,MEETING_DATE,MEETING_TYPE,ATTENDEE_COUNT,DURATION_MINUTES,MAIN_TOPIC,ASSIGNED_REP_ID,CREATED_AT)VALUES('"+midS+"','"+c.id+"','"+ds+"','"+mt+"',"+att+","+dur+",'"+tp+"','SR001',CURRENT_TIMESTAMP())"});
...
snowflake.execute({sqlText:"INSERT INTO T_MEETING_TRANSCRIPTS(TRANSCRIPT_ID,MEETING_ID,SEQUENCE_NUM,SPEAKER_LABEL,TRANSCRIPT_TEXT,CREATED_AT)VALUES('"+tridS+"','"+midS+"',"+(ti+1)+",'"+spk+"','"+tx+"',CURRENT_TIMESTAMP())"});}}}
...
snowflake.execute({sqlText:"INSERT INTO T_FINANCIAL_DATA(FINANCIAL_ID,COMPANY_ID,FISCAL_YEAR,REVENUE,OPERATING_INCOME,NET_INCOME,TOTAL_ASSETS,EQUITY,EMPLOYEE_COUNT,RETIREMENT_BENEFIT_OBLIGATION,CREATED_AT)VALUES('"+fid+"','"+c.id+"',"+yr+","+rev+","+op+","+net+","+ta+","+eq+","+ec+","+rb+",CURRENT_TIMESTAMP())"});}}
...
snowflake.execute({sqlText:"INSERT INTO T_CONTRACTS(CONTRACT_ID,COMPANY_ID,PRODUCT_ID,CONTRACT_STATUS,ANNUAL_PREMIUM,CONTRACT_START_DATE,CONTRACT_END_DATE,POLICY_NUMBER,CREATED_AT)VALUES('"+coidS+"','"+c.id+"','"+pIds[pIdx2]+"','契約中',"+prem+",'"+sy+"-04-01','"+(sy+3)+"-03-31','POL-"+coidS+"',CURRENT_TIMESTAMP())"});}}
...
-- ============================================================
CREATE STAGE IF NOT EXISTS NIPPONLIFE_DEMO_DB.RAW.PROPOSAL_EXPORT_STAGE;
...
def main(session, company_name: str, product_names: str, proposal_content: str) -> str:
    ...
    for f in stage_files:
        ...
        if safe_name in fname:
            actual_name = fname
            break
    url = session.sql(f"SELECT GET_PRESIGNED_URL({stage_path}, '{actual_name}', 3600) AS URL").collect()[0]['URL']
